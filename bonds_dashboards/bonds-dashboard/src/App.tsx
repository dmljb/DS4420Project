...
function App() {
  ...
  const fetchMinuteAnalysis = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5050/api/minute_analysis");
      if (!response.ok) {
  ...
  const fetchPredictionData = async () => {
    try {
      const response = await fetch("http://localhost:5050/api/yield_predictions");
  ...
  const handleAskAI = async () => {
    ...
    setRecoError(null);
    try {
      const response = await fetch("http://localhost:5050/api/recommendation");
